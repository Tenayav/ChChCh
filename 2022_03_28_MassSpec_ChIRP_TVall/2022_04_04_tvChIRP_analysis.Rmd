...
grViz(diagram = "digraph flowchart {
      ...
      [8]: 'Combine 3WT and 0KO Genes from 50ng (23 Genes) and 100ng (5 Genes)'
      [9]: 'Combine 2WT and 0KO Genes from 50ng (30 Genes) and 100ng (8 Genes)'
      [10]: 'Combine 3WT and 1KO Genes from 50ng (19 Genes) and 100ng (4 Genes)'
      [11]: 'Expand Comparison to Other Datasets'
      [12]: '50ng&100ng combined ratios and pvalues'
      [13]: 'Sucrose Gradient Ratios and KO checks'
      [14]: 'Odd and Even Ratios and KO Checks from Jan2022 TV'
      [15]: 'Odd and Even Ratios from Jan2022 Fede Data for both WT:KO and WT2h:KO'
      [16]: 'Michaels RNA seq Ratios and Padjvalues'
      [17]: '# Promising Genes - '
      ")
...
grViz(diagram = "digraph flowchart {
      ...
      [5]: 'Ratio WT:KO >= 1.5 [517 genes]'
      [6]: 'Cutoffs for 4 out of 6 WT Samples with Intensities above 0 [35 Genes]'
      ...
      [10]: 'Sucrose Gradient Ratios and KO checks'
      [11]: 'Odd and Even Ratios and KO Checks from Jan2022 TV'
      [12]: 'Odd and Even Ratios from Jan2022 Fede Data for both WT:KO and WT2h:KO'
      [13]: 'Michaels RNA seq Ratios and Padjvalues'
      [14]: '2 Promising Genes - Hmgb1 and Ddx39b '
...
pdf("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall//2022_04_13_scatter_wtmean_50ng_100ng.pdf")
ggplot(WTmean_50ng_100ng, aes(x=logwtmean.50ng, y=logwtmean.100ng)) + 
  geom_point()+
   geom_smooth(method=lm,  linetype="dashed", color="darkred", fill="blue")+
  labs(title="Mean of WT Intensities 100ng vs. 50ng ",
...
Intensities2_filteroutinput_nonzero_reorderedcolumns2 <- Intensities2_filteroutinput_nonzero_reorderedcolumns
Intensities2_filteroutinput_nonzero_reorderedcolumns2$ttest_pvalue <- sapply(1:nrow(Intensities2_filteroutinput_nonzero_reorderedcolumns2), function(i) t.test(as.numeric(as.character(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2[i,2:5]))), as.numeric(as.character(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2[i,6:11]))))[c("p.value")])
Intensities2_filteroutinput_nonzero_reorderedcolumns2$ttest_pvalue <- as.numeric(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2$ttest_pvalue))
Intensities2_filteroutinput_nonzero_reorderedcolumns2$wilcoxtest_pvalue <- sapply(1:nrow(Intensities2_filteroutinput_nonzero_reorderedcolumns2), function(i) wilcox.test(as.numeric(as.character(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2[i,2:5]))), as.numeric(as.character(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2[i,6:11]))))[c("p.value")])
Intensities2_filteroutinput_nonzero_reorderedcolumns2$wilcoxtest_pvalue <- as.numeric(unlist(Intensities2_filteroutinput_nonzero_reorderedcolumns2$wilcoxtest_pvalue))
...
ggplot(data=Intensities2_filteroutinput_nonzero_reorderedcolumns2, aes(x=logratio.combinedMarChIRP, y=ttest_pvalue, label=gene.combinedMarChIRP)) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
...
ggplot(data=Intensities2_filteroutinput_nonzero_reorderedcolumns2, aes(x=logratio.combinedMarChIRP, y=wilcoxtest_pvalue, label=gene.combinedMarChIRP)) +
        geom_point() + 
        theme_minimal() +
        geom_text_repel() +
        scale_color_manual(values=c("blue", "black", "red")) +
