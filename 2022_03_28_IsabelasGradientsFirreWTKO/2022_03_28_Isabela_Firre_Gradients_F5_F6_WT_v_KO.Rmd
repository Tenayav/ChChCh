...
test.funi <- function(Intensities_filter, col) { 

 ci <- combn(unique(Intensities_filter$genotype),2)
 sigsi <- list()
 for(i in 1:ncol(ci)) {
    sigsi[[i]] <- wilcox.test(
                   Intensities_filter[Intensities_filter$genotype == ci[1,i],col],
                   Intensities_filter[Intensities_filter$genotype == ci[2,i],col], exact=FALSE
                 )
    }
    names(sigsi) <- paste("Group",ci[1,],"by Group",ci[2,])
...
tests_Intensities_valid <- lapply(colnames(Intensities_fiter)[-1],function(x) test.funi(Intensities_filter,x))
names(tests_Intensities_valid) <- colnames(Intensities_valid)[-1]
```
