...
Intensities_filter_both_FC$logratio_mean_2 = log(Intensities_filter_both_FC$ratio_mean,2)
Intensities_filter_both_FC$logratio_mean50_2 = log(Intensities_filter_both_FC$ratio_50,2)
Intensities_filter_both_FC$logratio_mean100_2 = log(Intensities_filter_both_FC$ratio_100,2)

#make a histogram with FC distribution 
hist(Intensities_filter_both_FC$logratio_mean_2, breaks = 50, main="Frequency of Log Average of Fold Change of WT/KO Intensities \nfrom Grad2022 protein", xlab = "Log2 Average Fold Change of WT to KO with genes that have 4/4 WT intensity", ylab = "Frequency of Genes", labels=TRUE) 

hist(Intensities_filter_both_FC$logratio_mean50_2, breaks = 50, main="Frequency of Log of Fold Change of WT/KO Intensities \nfrom Grad2022 protein - 50 ng", xlab = "Log2 Fold Change of WT to KO with genes that have 4/4 WT intensity", ylab = "Frequency of Genes", labels=TRUE) 

hist(Intensities_filter_both_FC$logratio_mean100_2, breaks = 50, main="Frequency of Log of Fold Change of WT/KO Intensities \nfrom Grad2022 protein - 100 ng", xlab = "Log2 Fold Change of WT to KO with genes that have 4/4 WT intensity", ylab = "Frequency of Genes", labels=TRUE)
...
```{r - filter by FC}
#filter by log2FC = 0.58 means FC 1.5
Intensities_filter_both_FC_filter <- Intensities_filter_both_FC %>%
  filter(logratio_mean_2 >= 0.5849 | logratio_mean_2 <= -0.5849)
#425 genes

Intensities_filter_both_FC_UP <- Intensities_filter_both_FC %>%
  filter(logratio_mean_2 >= 0.5849)
#347
...
#there's a very distinguish group of genes with logFC > 10, let's see who they are
Intensities_filter_both_FC_UP_high <- Intensities_filter_both_FC_UP %>%
  filter(logratio_mean_2 > 10)
#save this interesting list
write.csv(Intensities_filter_both_FC_UP_high, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensity_FC_upWT_high.csv")

Intensities_filter_both_FC_DOWN <- Intensities_filter_both_FC %>%
  filter(logratio_mean_2 <= -0.5849)
#78
```

```{r - quick look at Chchch data}
#now let's see if these 347 genes UP in WT appear in the Chchch data
#read in raw data from ChChCh
ch_50ng_raw <- read.csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall/2022_03_24_50ng_protein_ChIRP_5percentFDR_JRinn.csv")
ch_100ng_raw <- read.csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall/2022_03_24_100ng_ChIRP_5percentFDR_JRinn_Results.csv")

ch_50ng_grad <- intersect(Intensities_filter_both_FC_UP$gene, ch_50ng_raw$Gene)
ch_50ng_grad <- as.data.frame(ch_50ng_grad)
#245
#let's see if there are reads
colnames(ch_50ng_grad) <- c("Gene")
ch_50ng_grad <- ch_50ng_grad %>%
  left_join(ch_50ng_raw)
#Ddb1 // Hspa5 // Rap1b

ch_100ng_grad <- intersect(Intensities_filter_both_FC_UP$gene, ch_100ng_raw$Gene)
ch_100ng_grad <- as.data.frame(ch_100ng_grad)
#252
colnames(ch_100ng_grad) <- c("Gene")
ch_100ng_grad <- ch_100ng_grad %>%
  left_join(ch_100ng_raw)

ch_grad_both <- intersect(ch_50ng_grad$Gene, ch_100ng_grad$Gene)
#217

#now compare to Tenaya's filter 3 out of 6 in WT (Chchch data)
#read in
ch_filter_3of6 <- read.csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall/2022_04_11_pvalues_ttest_Wilcoxon_Intensities_3_out_of_6_Mar2022.csv")
ch_filter_grad <- intersect(Intensities_filter_both_FC_UP$gene, ch_filter_3of6$gene)
#3 genes
```
...
#filter using Michael's parameter for the firre paper (FC 1.5, but log2FC 0.5849)
rna_seq_down <- rna_seq %>%
  filter(padj <= 0.05, log2FoldChange <= -0.5849)
#314 genes

#comparing what is DOWN in KO cells with what we are considering UP/enriched in WT in MS data
intersect_rna_seq <- intersect(Intensities_filter_both_FC_UP$gene, rna_seq_filtered2$gene_name)
#Apoe gene - only 1

###so far just one gene DEG in KO cells intersect the results from GRAD-MS proteins, that's a good sign.

####NOT WORKING - WORKING HERE
#Venn diagram
venn.diagram(
        x = list(Intensities_filter_both_FC_UP$gene, rna_seq_filtered2$gene_name),
        category.names = c("MS_up" , "RNA-seq_down "),
        filename = 'venn_diagramm.png',
        output=TRUE)

pdf("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/venn_MS_rnaseq.pdf")

venn_plot <- draw.pairwise.venn(length(Intensities_filter_both_FC_UP$gene),
                                length(rna_seq_filtered2$gene_name),
                                length(intersect_rna_seq),
                                category = c("MS","RNA-seq"),
                                scaled=TRUE,
...
#let's see overall DEG
rna_seq_filtered3 <- rna_seq %>%
  filter(padj <= 0.05, log2FoldChange >= 0.5849 | log2FoldChange <= -0.5849)

intersect2 <- intersect(Intensities_filter_both_FC_filter$gene, rna_seq_filtered3$gene_name)
intersect2 <- as.data.frame(intersect2)
intersect2 <- intersect2 %>%
  dplyr::rename(gene = intersect2)
#7 genes


###CLEAN UP THIS
intersect2 <- intersect2 %>%
  left_join(Intensities_filter_both_FC_filter)

rna_seq_filtered3 <- rna_seq_filtered3 %>%
  dplyr::rename(gene = gene_name)

intersect2 <- intersect2 %>%
  left_join(rna_seq_filtered3)
```

```{r - before stats, filter by intensity}
#read in
Intensities_filter_both <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensities_valid_filter.csv")

#or use filtered by FC
Intensities_filter_both <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensity_FC_upWT.csv")

#filter #1 = KO < WT in both replicates separetly
Intensities_filter_both <- Intensities_filter_both %>%
  mutate(sum50_ko = rowSums(Intensities_filter_both[ , c(2,3)])) %>%
  mutate(sum50_wt = rowSums(Intensities_filter_both[ , c(4,5)])) %>%
  mutate(sum100_ko = rowSums(Intensities_filter_both[ , c(6,7)])) %>%
  mutate(sum100_wt = rowSums(Intensities_filter_both[ , c(8,9)]))

Intensities_filter_both_filter <- Intensities_filter_both %>%
...
Intensities_filter_both_filter.1 <- Intensities_filter_both_filter %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 1 | ptn_mesc_ko_p2_f5_f6_intensity.50ng == 1 | ptn_mesc_ko_p1_f5_f6_intensity.100ng == 1 | ptn_mesc_ko_p2_f5_f6_intensity.100ng == 1)
#159 genes

#now filtering considering occurence in both replicates for the same run
Intensities_filter_both_filter.2 <- Intensities_filter_both_filter %>%
 filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 1 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 1 | ptn_mesc_ko_p1_f5_f6_intensity.100ng == 1 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 1)
#62 genes

#now filtering considering occurence in same replicates for both run
Intensities_filter_both_filter.3 <- Intensities_filter_both_filter %>%
 filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 1 & ptn_mesc_ko_p1_f5_f6_intensity.100ng == 1 | ptn_mesc_ko_p2_f5_f6_intensity.50ng == 1 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 1)
#38 genes

#now filtering intensity = 0 in all KO
Intensities_filter_both_filter.4 <- Intensities_filter_both_filter %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 1 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 1 & ptn_mesc_ko_p1_f5_f6_intensity.100ng == 1 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 1)
#4
```


```{r - run Wilcox function}
#read in
Intensities_filter_both <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensities_valid_filter.csv")
...
#plot log10 pval
results_Intensities_valid$log_p <- (log(results_Intensities_valid$p,10)* -1)
hist(results_Intensities_valid$log_p, breaks = 20, main="Frequency of Wilcoxon Rank Sum Test P-values \nComparing KO with WT Mass Spec Intensities from SucGrad F5&6 Protein - NEW FILTER", xlab = "-log(P-value) from Wilcoxon Rank Sum Test", ylab = "Frequency of Genes", labels=TRUE) 
...
#read in if needed
sig_Int_genes_filter <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sigInten_valid_genes_filter_values.csv")
...
#60 genes
#save this list
write_csv(sig_Int_genes_filter_5, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sig_Int_genes_filter_5.csv")

#how this compare with what we get only filtering by FC (not pval, any stat test)
#intensity = 0 in at least one of KO replicates in any run
intersect <- intersect(sig_Int_genes_filter_5$gene, Intensities_filter_both_filter.1$gene)
...
#21 genes
intersect_2 <- intersect(intersect$intersect,sig_Int_genes_filter_5.1$gene)
...
sig_Int_genes_filter_5.3 <- sig_Int_genes_filter_5 %>% 
 filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 | ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#10 genes
...
###compare this with filtered without pval
#intensity = 0 in at least one of KO replicates in any run
intersect1.1 <- intersect(Intensities_filter_both_filter.1$gene,sig_Int_genes_filter_5.1$gene)  
#21 genes

#now filtering considering occurence in both replicates for the same run
intersect1.2 <- intersect(Intensities_filter_both_filter.2$gene,sig_Int_genes_filter_5.2$gene)  
#14 genes

#now filtering considering occurence in same replicates for both run
intersect1.3 <- intersect(Intensities_filter_both_filter.3$gene,sig_Int_genes_filter_5.3$gene)  
#10 genes

#now filtering intensity = 0 in all KO
intersect1.4 <- intersect(Intensities_filter_both_filter.4$gene,sig_Int_genes_filter_5.4$gene)  
#4
```

##these filter might be applied before Wilcox test or after, it doesn't matter, results are the same

```{r - let's add pval from Wilcoxon in the list filtered only by FC}
#read in - list filtered only by FC
Intensities_filter_both_FC_UP <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensity_FC_upWT.csv")

#read in - wilcoxon p values
...
#merge
Intensities_FCfilter_pval <- Intensities_filter_both_FC_UP %>%
  left_join(results_Intensities_valid)

#let's see pval distribution
Intensities_FCfilter_pval$log_p <- (log(Intensities_FCfilter_pval$p,10)* -1)
hist(Intensities_FCfilter_pval$log_p, breaks = 20, main="Frequency of Wilcoxon Rank Sum Test P-values \nComparing KO with WT Mass Spec Intensities from SucGrad F5&6 Protein - filtered by FC previously", xlab = "P-value from Wilcoxon Rank Sum Test", ylab = "Frequency of Genes", labels=TRUE) 

#filter by pval 0.05
Intensities_FCfilter_pval_filter <- Intensities_FCfilter_pval %>%
  filter(p <= 0.05)
#45 genes

#save this list
write_csv(Intensities_FCfilter_pval_filter, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/Intensities_FCfilter_pval_filter.csv")
...
#now let's see if these 45 genes appear in the Chchch data
#read in raw data from ChChCh
ch_50ng_raw <- read.csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall/2022_03_24_50ng_protein_ChIRP_5percentFDR_JRinn.csv")
ch_100ng_raw <- read.csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_MassSpec_ChIRP_TVall/2022_03_24_100ng_ChIRP_5percentFDR_JRinn_Results.csv")

ch_50ng_grad <- intersect(Intensities_FCfilter_pval_filter$gene, ch_50ng_raw$Gene)
ch_50ng_grad <- as.data.frame(ch_50ng_grad)
#34
#let's see if there are reads
colnames(ch_50ng_grad) <- c("Gene")
...
ch_100ng_grad <- intersect(Intensities_FCfilter_pval_filter$gene, ch_100ng_raw$Gene)
ch_100ng_grad <- as.data.frame(ch_100ng_grad)
#34
colnames(ch_100ng_grad) <- c("Gene")
ch_100ng_grad <- ch_100ng_grad %>%
...
sig_Int_genes_filter_5 <- sig_Int_genes_filter_5 %>%
  mutate(mean50_ko = rowMeans(sig_Int_genes_filter_5[ , c(6,7)])) %>%
  mutate(mean50_wt = rowMeans(sig_Int_genes_filter_5[ , c(8,9)])) %>%
  mutate(mean100_ko = rowMeans(sig_Int_genes_filter_5[ , c(10,11)])) %>%
  mutate(mean100_wt = rowMeans(sig_Int_genes_filter_5[ , c(12,13)])) 
...
#what if we calculate the mean ratio
sig_Int_genes_filter_5$mean_ratio <- rowMeans(sig_Int_genes_filter_5[ , c(22,23)])
sig_Int_genes_filter_5$logratio_mean_2 = log(sig_Int_genes_filter_5$mean_ratio,2)
sig_Int_genes_filter_5.5 <- sig_Int_genes_filter_5 %>%
  filter(logratio_mean_2 >= 0.5849)
#45 genes

#compare order of filtering = FC > wilcox // wilcox > FC
intersect <- intersect(Intensities_FCfilter_pval_filter$gene, sig_Int_genes_filter_5.5$gene)
...
#this filter considered wilcox pval < 0.05, KO < WT, FC > 1.5 (simple ratio) = 31 genes
...
#now if we compare to one step above of filtering = 60 genes
#pval < 0.05, KO < WT
sig_Int_genes_filter_5 <- read_csv("/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sig_Int_genes_filter_5.csv")

intersect_deseq3 <- intersect(res_filter2$gene, sig_Int_genes_filter_5$gene)
intersect_deseq3 <- as.data.frame(intersect_deseq3)
#same 8

#filter same log2FC than others
res_filter3 <- res_merged %>%
  filter(padj <= 0.05, log2FoldChange >= 0.5849)
#9 genes

#compare with 45 genes filtered by FC and wilcoxon pval
intersect_3 <- intersect(res_filter3$gene, Intensities_FCfilter_pval_filter$gene )
intersect_3 <- as.data.frame(intersect_3)
#8 genes

#compare this with FC filter, wilcoxon pval, intensity = 0
intersect1.10 <- intersect(intersect_3$intersect_3, intersect1.1)
intersect1.10 <- as.data.frame(intersect1.10)
#4 genes
```
...
#filter using Michael's parameter for the firre paper (FC 1.5, but log2FC 0.5849)
rna_seq_filtered2 <- rna_seq %>%
  filter(padj <= 0.05, log2FoldChange <= -0.5849)
#314 genes

intersect_rna_seq <- intersect(sig_Int_genes_filter_5$gene, rna_seq_filtered2$gene_name)
#none

#merge rna-seq data with the TOP hits list = 60 genes
rna_seq <- rna_seq %>%
  dplyr :: rename(gene = gene_name)

wilcox_rna_seq <- sig_Int_genes_filter_5 %>%
  left_join(rna_seq)

wilcox_rna_seq <- wilcox_rna_seq %>%
  filter(log2FoldChange <= -0.415)

###so far any gene DEG in KO cells intersect the results from GRAD-MS proteins, that's a good sign.
```

#work more with other filters - maybe not use pval from wilcox - only FC and then see pval.
```
