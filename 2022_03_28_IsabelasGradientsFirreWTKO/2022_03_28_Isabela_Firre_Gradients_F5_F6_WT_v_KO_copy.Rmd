...
#list of 2317 genes to start comparison with KO
```
...
```{r - try some other filters - intensity = 0}
#from the 808 gene list considering pval 0.05, let's try to filter by intensities = 0 in KO
sig_Int_genes_filter_2 <- sig_Int_genes_filter %>%
...
#82 genes that has intensity = 0 in at least one of KO replicates in any run

#now filtering considering occurence in both replicates for the same run
sig_Int_genes_filter_3 <- sig_Int_genes_filter %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 | ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#23 genes

#now filtering intensity = 0 in all KO
sig_Int_genes_filter_4 <- sig_Int_genes_filter %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#none
```
...
```{r - comparing ITP vs TKV final list}
#filters in both: pval <= 0.05 & sum_ko < sum_wt #significantIntensities_valid_genes_doublefilter
#filter plus ITP: Int_WT > 0 in all replicates/runs #sig_Int_genes_filter_5

#bring ptn names to lower cap
sig_Int_genes_filter_5 <- sig_Int_genes_filter_5 %>%
  mutate(ptn_name = tolower(sig_Int_genes_filter_5$gene))

intersect_itp_tkv <- intersect(sig_Int_genes_filter_5$ptn_name, significantIntensities_valid_genes_doublefilter$ptn_name)

#none!!!!!!!!!!
```

```{r - filtering further what I found - out of 245 genes}
...
QUESTION: should we apply these filters before running wilcox? 

```{r - merge KO values - filter KO > WT - run function}
Intensities_filter_both <- Intensities_WT_filter_both %>%
  left_join(Intensities_50ng) %>%
  left_join(Intensities_100ng)

#filter by KO > WT (sum of each run)
...
Intensities_filter_both <- Intensities_filter_both %>%
  filter(sum50_ko < sum50_wt & sum100_ko < sum100_wt) %>%
  select(!sum50_ko) %>% select(!sum50_wt) %>% select(!sum100_ko) %>% select(!sum100_wt)
#744 gene to start!
...
Intensities_filter <- Intensities_filter[-c(1), ]
genotype <- c("WT","WT","WT","WT","KO","KO","KO","KO")
Intensities_filter <- Intensities_filter %>% add_column(genotype = genotype, .before = "Aars1")
...
results_Intensities_valid_filter2 <- results_Intensities_valid 

#filter by pval - by Tenaya's code
significant <- c("very", "significant")
pattern_to_search <- paste0(significant, collapse = "|")

results_Intensities_valid_filter2 <- results_Intensities_valid_filter2 %>% 
  mutate(significance = ifelse(p < 0.05, "very",
                          ifelse(p < 0.10, "significant", "not")))

significantIntensities_valid_genes_filter <- results_Intensities_valid_filter2[grepl(pattern_to_search, results_Intensities_valid_filter2$significance), ]

#filter by pval value
sig_Int_genes_filter2 <- results_Intensities_valid_filter2 %>%
  filter(p <= 0.05)
#246 genes

#merge intensities values in the same table
sig_Int_genes_filter2 <- sig_Int_genes_filter2 %>%
  rownames_to_column(var = "gene") %>%
  left_join(Intensities_50ng) %>%
  left_join(Intensities_100ng)

#save this table
write_csv(sig_Int_genes_filter2, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sigInten_valid_genes_filter2_values.csv")

#now let's filter after per intensity = 0
#intensity = 0 in at least one of KO replicates in any run
sig_Int_genes_filter2.1 <- sig_Int_genes_filter2 %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 | ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 | ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 | ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#48 genes

#now filtering considering occurence in both replicates for the same run
sig_Int_genes_filter2.2 <- sig_Int_genes_filter2 %>%
 filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 | ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#16 genes

#save this as my top hits for now
write_csv(sig_Int_genes_filter2.2, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sig_Int_genes_filter2.2.csv")

#how this overlap with the other filter
intersect_filters <- intersect(sig_Int_genes_filter2.2$gene, sig_Int_genes_filter_5.2$gene)
#all 14 ===== DOESN'T MATTER FILTER KO > WT BEFORE OR AFTER WILCOX

#now filtering intensity = 0 in all KO
sig_Int_genes_filter2.3 <- sig_Int_genes_filter2 %>%
  filter(ptn_mesc_ko_p1_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.50ng == 0 & ptn_mesc_ko_p1_f5_f6_intensity.100ng == 0 & ptn_mesc_ko_p2_f5_f6_intensity.100ng == 0)
#none
```
...
#no intensity = 0 apllied here
#starting with 249 genes 

sig_Int_genes_filter2 <- sig_Int_genes_filter2 %>%
  mutate(mean50_ko = rowSums(sig_Int_genes_filter2[ , c(6,7)])) %>%
  mutate(mean50_wt = rowSums(sig_Int_genes_filter2[ , c(8,9)])) %>%
  mutate(mean100_ko = rowSums(sig_Int_genes_filter2[ , c(10,11)])) %>%
  mutate(mean100_wt = rowSums(sig_Int_genes_filter2[ , c(12,13)])) 

sig_Int_genes_filter2 <- sig_Int_genes_filter2 %>%
  mutate(ratio_50 = sig_Int_genes_filter2$mean50_wt/sig_Int_genes_filter2$mean50_ko) %>%
  mutate(ratio_100 = sig_Int_genes_filter2$mean100_wt/sig_Int_genes_filter2$mean100_ko)

#very stringent filter
sig_Int_genes_filter2.4 <- sig_Int_genes_filter2 %>%
  filter(!ratio_50 < 1.5 & !ratio_100 < 1.5)
#15 genes

#save this as my top hits for now
write_csv(sig_Int_genes_filter2.4, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sig_Int_genes_filter2.4.csv")

intersect2.4 <- intersect(sig_Int_genes_filter2.2$gene, sig_Int_genes_filter2.4$gene)
#3 genes
```

#TODO: filter or compare at some point the list with ko x wt RNA-seq data

```{r - rerun Tenaya's first list - with my clean up and way of doing but using her strategy}
#50 ng protein
mass_spec_50ng_Mar2022_SucGrad <- read_csv("/scratch/Shares/rinn/tenaya/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/2022_03_24_50ng_protein_SCP_JRinn_PTN_MESC_results.csv") %>% clean_names()
...
#filtering only Intensities values
Intensities_50ng <- mass_spec_50ng_Mar2022_SucGrad[,c(4,28,29,30,31)]
colnames(Intensities_50ng)[2:5] <- paste(colnames(Intensities_50ng)[2:5],"50ng",sep=".")
...
```
#filter initial list using only WT values: sum of intensity > 0 considering the 4 replicates (P1/P2, 50ng/100ng)
#filter by sum of rows
```{r - filter based on WT occurance}
#keep only WT
Intensities_50ng_WTcheck <- Intensities_50ng[,c(1,4,5)]
Intensities_100ng_WTcheck <- Intensities_100ng[,c(1,4,5)]
#join them
...
#save this initial table 3945 genes + intensities values
write_csv(Intensities_valid, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/intensities_valid_TKV.csv")
#obs:none changes so far
```

```{r - prep for run function/wilcox test}
Intensities_valid <- as.data.frame(t(Intensities_valid))
colnames(Intensities_valid) <- unlist(Intensities_valid[1, ], use.names = F)
Intensities_valid <- Intensities_valid[-c(1), ]
genotype <- c("WT","WT","WT","WT","KO","KO","KO","KO")
Intensities_valid <- Intensities_valid %>% add_column(genotype = genotype, .before = "A2m", name_repair = "minimal")
Intensities_valid <- Intensities_valid %>%
  select(!genotype.1) %>%
  select(!name_repair)
Intensities_valid[] <- lapply(Intensities_valid, type.convert, as.is = TRUE)
...
#save 
results_Intensities_valid_2 <- do.call(rbind.data.frame, tests_Intensities_valid)
results_Intensities_valid_2 <- results_Intensities_valid_2 %>%
  rownames_to_column(var ="gene")
write.csv(results_Intensities_valid_2,"/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/results_protein_Intensities_2.csv")

#bind intensities values
results_Intensities_valid_2 <- results_Intensities_valid_2 %>%
  left_join(Intensities_50ng) %>%
  left_join(Intensities_100ng)

#save this table
write_csv(results_Intensities_valid_2, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/sigInten_valid_genes_values.csv")
```

```{r - comparing results}
#Tenaya's filter at least one intensity WT >0
results_Intensities_valid_2 <- results_Intensities_valid_2 %>%
 filter(p <= 0.05)
#93 genes

results_Intensities_valid_2_doublefilter <- results_Intensities_valid_2 %>%
  mutate(sum50_ko = rowSums(results_Intensities_valid_2[ , c(6,7)])) %>%
  mutate(sum50_wt = rowSums(results_Intensities_valid_2[ , c(8,9)])) %>%
  mutate(sum100_ko = rowSums(results_Intensities_valid_2[ , c(10,11)])) %>%
  mutate(sum100_wt = rowSums(results_Intensities_valid_2[ , c(12,13)]))

results_Intensities_valid_2_doublefilter <- results_Intensities_valid_2_doublefilter %>%
  filter(sum50_ko < sum50_wt & sum100_ko < sum100_wt)
#60 genes

#compare the 2 strategies
intersect_itp_tkv <- intersect(sig_Int_genes_filter_5$gene, results_Intensities_valid_2_doublefilter$gene)
```
