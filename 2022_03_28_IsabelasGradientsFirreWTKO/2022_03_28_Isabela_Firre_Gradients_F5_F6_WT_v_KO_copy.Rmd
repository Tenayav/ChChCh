...
##here to below
#run DESeq
rlog_counts <- DESeqDataSetFromMatrix(counts, samplesheet_grad, 
                                      design = ~ genotype) %>%
 # rlog() %>%
  assay() %>%
  as.data.frame() %>% 
...
#save for futute read in
write_csv(rlog_counts, "/scratch/Shares/rinn/isabela/ChChCh/2022_03_28_IsabelasGradientsFirreWTKO/results/rlog_counts_deseq.csv")
##these are normalized counts that can be used in many other analysis for now on
dds <- DESeq(rlog_counts)
res1 <- results(dds)
res <- lfcShrink(dds, coef="genotype_WT_vs_KO")
```
```
