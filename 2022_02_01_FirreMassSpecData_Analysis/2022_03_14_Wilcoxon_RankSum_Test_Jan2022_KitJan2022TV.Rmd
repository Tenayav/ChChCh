...
#Dataframes of common and all significant genes
```{r}
...
#Need to have the box-whisker plots from my old way of doing this to check that I believe the significance
#Also need to find the ones that are not significant for KO...how do I do that

#For common genes, I'm going to do those box-whisker plots 
```{r}
...
#Spectral
sig_genes <- c("anxa1","atp5f1a","btf3","ckb","gapdh","hdlbp","hnrnpm","krt14","krt16","krt5","mif","naa15","pabpc1","pkp1","rpl10$","rpl38","slc25a5")
...
ggplot(mm_sigSpectral)+geom_boxplot(aes(x=genotype, y=value),fill = "#00AFBB")+facet_grid(.~variable)+stat_compare_means(label.x = 0.5)
dev.off()
...
ggplot(mm_sigIntensities)+geom_boxplot(aes(x=genotype, y=value),fill = "#00AFBB")+facet_grid(.~variable)+stat_compare_means(label.x = 0.5)
dev.off()
...
for (var in mm_sigIntensities$variable) { 
  ...
  print(ggpaired(mm_sigIntensities[mm_sigIntensities$variable==var,],x="genotype",y="value",fill="#00AFBB",line.color ="gray",xlab = "Genotype of Firre RNA", ylab = "Mass Spec Intensities")+ggtitle(var)+stat_compare_means(label.x = 0.5))
  dev.off()
}
