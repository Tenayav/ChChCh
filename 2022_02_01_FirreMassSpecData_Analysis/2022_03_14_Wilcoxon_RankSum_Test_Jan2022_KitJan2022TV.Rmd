...
test.fun <- function(Spectral, col) { 
 ...
 for(i in 1:ncol(c1)) {
    sigs[[i]] <- wilcox.test(
                   ...
                   Spectral[Spectral$genotype == c1[2,i],col]
                 )
    }
...
test.fun2 <- function(Intensities, col) { 
 ...
 c2 <- combn(unique(Intensities$genotype),2)
 sigs2 <- list()
 for(i in 1:ncol(c2)) {
    sigs2[[i]] <- wilcox.test(
                   Intensities[Intensities$genotype == c2[1,i],col],
                   Intensities[Intensities$genotype == c2[2,i],col]
                 )
    }
...
tests_Intensities <- lapply(colnames(Intensities)[-1],function(x) test.fun(Intensities,x))
